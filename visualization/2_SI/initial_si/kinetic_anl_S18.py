...
df = pd.read_excel(file_path, sheet_name='Fig. S18', skiprows=1)
...
def plot_partial_pressures(df):
    ...
    plt.ylabel("Specific activity\n(μmolCO$_{2}$/g$_{cat}$s$^{-1}$))", font_properties=font_properties_label)
    plt.xscale('log')
    plt.yscale('log')
    plt.ylim(10,3000)
    plt.xlim(0.5,20)
    ...
    plt.yticks([])
    plt.tight_layout()
    plt.savefig(f'./1_pngs/fig S18.png', dpi=200)
    plt.show()
