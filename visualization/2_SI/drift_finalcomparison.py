...
def combined(base_directories, output_path, output_filename, csv_files, csv_files2, xtick_onlyone=False,time_points=[],interval=1):
    ...
    # First pass can be removed since we're only plotting one time point
    y_max = 0.4
    
    # Remove colorbar setup since we don't need it anymore
    ...
    for (dataset_name, base_directory), (ax1, ax2) in zip(base_directories.items(), axes_pairs):
        ...
        print(len(all_data2))
        # Only plot the final time point
        final_time = time_points[0]
        for data in all_data:
            ...
            absorbance_values = [get_absorbance(data, wave, final_time) 
        ...
        for data in all_data2:
            ...
            for wave, value in zip(peak_wavenumbers, peak_values):
                if wave < 2300:
                    ax1.plot([wave, wave], [value-0.01, value+0.03], color='red', linestyle='--', linewidth=0.8, alpha=0.5)
                    ax1.text(wave, value+0.03, f'{wave:.0f}', fontproperties=font_properties_annotate, color='red', ha='center', va='bottom')
            ...
            peak_wavenumbers, peak_values = find_local_maxima(
                selected_wavenumbers, absorbance_values)
            for wave, value in zip(peak_wavenumbers, peak_values):
                ax2.plot([wave, wave], [value-0.01, value+0.1], color='red', linestyle='--', linewidth=0.8, alpha=0.5)
                ax2.text(wave, value+0.1, f'{wave:.0f}', fontproperties=font_properties_annotate, color='red', ha='center', va='bottom')
    ...
    for (ax1, ax2), is_last_row in zip(axes_pairs, [False, False, True]):
        ...
        ax1.set_xticks(np.arange(0, 60, 1))
...
if __name__ == "__main__":
    ...
    base_directories, csv_files2 = get_base_directories('co_o2')
    fig_e, (ax_e1, ax_e2, ax_e3) = combined(
        base_directories=base_directories,
        output_path='./output',
        output_filename='test2.png',
        csv_files=csv_files,
        csv_files2=csv_files2,
        xtick_onlyone=True,time_points=np.arange(0,60,1),interval=10  # Set your preferred value here
    )