...
def combined(base_directories, output_path, output_filename, csv_files, xtick_onlyone=False,time_points=[]):
            ...
            for data in all_data:
                ...
                if time == 15:
                    ...
                    for wave, value in zip(peak_wavenumbers, peak_values):
                        ax1.plot([wave, wave], [value-0.01, value+0.01], color='black', linestyle='--', linewidth=0.8, alpha=0.5)
                        ax1.text(wave, value+0.01, f'{wave:.0f}', fontproperties=font_properties_annotate, color='black', ha='center', va='bottom')
                ...
                if time == 15:
                    peak_wavenumbers, peak_values = find_local_maxima(
                        selected_wavenumbers, absorbance_values)
                    for wave, value in zip(peak_wavenumbers, peak_values):
                        ax2.plot([wave, wave], [value-0.01, value+0.01], color='black', linestyle='--', linewidth=0.8, alpha=0.5)
                        ax2.text(wave, value+0.01, f'{wave:.0f}', fontproperties=font_properties_annotate, color='black', ha='center', va='bottom')
...
if __name__ == "__main__":
    ...
    fig_d, (ax_d1, ax_d2, ax_d3) = combined(
        ...
        csv_files=csv_files,
        xtick_onlyone=True,time_points=np.arange(0,15,1) # Set your preferred value here
    )
    ...
    fig_e, (ax_e1, ax_e2, ax_e3) = combined(
        ...
        csv_files=csv_files,
        xtick_onlyone=True,time_points=np.arange(0,90,1)  # Set your preferred value here
    )
