...
def plot_co_conversion(df,color,title):
    ...
    # Sample data columns
    sample_labels = ['Ar','CO', 'O2', 'CO+O2']
    reference_labels = ['Au2O3', 'Au metal']
    show_labels=['Ar',  'CO',  'O$_2$','CO+O$_2$',]
    show_reference_labels = ['Au$_2$O$_3$', 'Au metal']
    ...
    plt.tight_layout()
    plt.savefig(f'./1_pngs/Fig. S23{title}.png', dpi=200)
    plt.show()
...
if __name__ == "__main__":
    ...
    df2 = pd.read_excel(file_path, sheet_name='Fig. S23b', skiprows=1)
    df3 = pd.read_excel(file_path, sheet_name='Fig. S23c', skiprows=1)
    plot_co_conversion(df,colors[0],title='a')
    plot_co_conversion(df2,colors[1],title='b')
    plot_co_conversion(df3,colors[2],title='c')