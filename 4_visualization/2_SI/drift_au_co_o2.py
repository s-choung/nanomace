...
def get_base_directories(dataset_type):
    """
    Get the base directories for different datasets.
    
    Args:
        dataset_type (str): Either 'co' or 'co_o2' to specify which dataset to use
        
    Returns:
        dict: Dictionary of dataset paths
        list: List of CSV files to process
    """
    ...
    if dataset_type == 'co_o2':
        ...
        raise ValueError("dataset_type must be either 'co' or 'co_o2'")
...
def create_combined_plot(base_directories, output_path, output_filename, csv_files):
    """
    Create combined DRIFTS plots with dynamic y-axis scaling.
    
    Args:
        base_directories (dict): Dictionary of dataset names and their paths
    ...
    cmap = plt.cm.inferno
    time_points = np.arange(0, 76, 1)
    norm = plt.Normalize(min(time_points), max(time_points))
    time_colors = cmap(np.linspace(0, 0.8, len(time_points)))
    
    # Remove the max_time tracking since we're using fixed time points
    max_absorbance = 0
    
    # First pass: collect data and find maximum absorbance
    for dataset_name, base_directory in tqdm(base_directories.items(), desc="Finding max absorbance"):
        all_data = []
        ...
        for time in time_points:  # Use fixed time_points here
    ...
    # Update colorbar normalization to use actual max time
    ...
    for (dataset_name, base_directory), (ax1, ax2) in tqdm(zip(base_directories.items(), axes_pairs), 
                                                          desc="Plotting data", 
                                                          total=len(base_directories)):
        all_data = []
        ...
        for filename in csv_files:
            ...
            data = read_csv(path, header=1).dropna(axis=0)
            all_data.append(data)
        # Remove the time points redefinition - use the fixed time points from above
...
if __name__ == "__main__":
    ...
    # Plot CO-O2 dataset
    co_o2_directories, co_o2_files = get_base_directories('co_o2')
    create_combined_plot(
        base_directories=co_o2_directories,
        output_path='./2_drifts',
        output_filename='au_combined_drifts_co_o2.png',
        csv_files=co_o2_files
    )
