...
df = pd.read_excel(file_path, sheet_name='Fig. S9', skiprows=1)
print(df)
def plot_xrd(df):
    plt.figure(figsize=(5, 4))
    labels =df.columns[[1,3,5,7,9]]
    references =df.columns[[11]]
    for i, label in enumerate(labels):
        x = df["Degree (2-theta)"]
        y = df[label]
        plt.plot(x, y, label=label, color='black', linewidth=1.5, zorder=2-0.1*i)
    for i, reference in enumerate(references):
        for j, degree in enumerate(df.columns[[10]]):
            x = df[degree]
            y = df[reference]
    ...
    plt.savefig(f'./1_pngs/Fig. S9.png', dpi=200)
    plt.show()
