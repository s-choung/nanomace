...
def e(
    ...
    for filename, ax in zip(csv_files, axes):
        ...
        time = time[::1]  ############ Use every second time point
        wavenumber = wavenumber[::1]  ############ Use every second wavenumber
    ...
    '''    # Add arrows above the plot
        # First arrow: 0 to 15
        axes[0].annotate('', 
            xy=(0, 1.15), xytext=(0.25, 1.15),
            xycoords='axes fraction',
            arrowprops=dict(arrowstyle='<-', color='black', linewidth=1))
        
        # Second arrow: 15 to 45
        axes[0].annotate('', 
            xy=(0.26, 1.15), xytext=(0.75, 1.15),
            xycoords='axes fraction',
            arrowprops=dict(arrowstyle='<-', color='black', linewidth=1))
        
        # Third arrow: 45 to 60
        axes[0].annotate('', 
            xy=(0.76, 1.15), xytext=(1.0, 1.15),
            xycoords='axes fraction',
            arrowprops=dict(arrowstyle='<-', color='black', linewidth=1))
        
        # Add text labels above arrows
        axes[0].annotate('CO(g)', 
            xy=(0.01, 1.2), xycoords='axes fraction',
            ha='left', va='bottom', 
            fontproperties=font_properties_tick)
        axes[0].annotate('Ar(g)', 
            xy=(0.26, 1.2), xycoords='axes fraction',
            ha='left', va='bottom',
            fontproperties=font_properties_tick)
        axes[0].annotate('O$_2$(g)', 
            xy=(0.76, 1.2), xycoords='axes fraction',
            ha='left', va='bottom',
            fontproperties=font_properties_tick)
    '''
    ...
    fig.supylabel('Wavenumber $(cm^{-1})$', fontproperties=font_properties_label, x=0.02, y=0.5)

    # Add colorbar (modified position and style)
    cbar_ax = fig.add_axes([0.75, 0.93, 0.15, 0.02])  # [left, bottom, width, height]
    # Get the colormap object
    cmap = plt.get_cmap(colormap)
    #axes[0].text(0.72, 1.44, 'Weak', fontproperties=font_properties_annotate, color=cmap(0.2), transform=axes[0].transAxes)
    #axes[0].text(0.98, 1.44, 'Strong', fontproperties=font_properties_annotate, color=cmap(0.9), transform=axes[0].transAxes)
    ...
    cbar.ax.set_xlabel('Absorbance (a.u.)',  fontproperties=font_properties_label, labelpad=5)
    cbar.ax.xaxis.set_label_position('top')
    plt.savefig(f'./output/e.png', dpi=200)
    plt.show()
